    ...
    std::function<uint8_t(size_t, size_t, uint8_t)> pixelFilter)
{
    ...
    memcpy(bm.fh.signature, "BM", 2);
    bm.fh.filesize = fileSize;
    bm.fh.fileoffset_to_pixelarray = offsetPixels;
    bm.bih.dibheadersize = sizeof(bitmapinfoheader);
    bm.bih.width = width;
    bm.bih.height = height;
    bm.bih.planes = 1;
    bm.bih.bitsperpixel = bitsPerPixel;
    bm.bih.compression = 0;
    bm.bih.imagesize = pixelBytes;
    bm.bih.ypixelpermeter = 0x130B; //2835 , 72 DPI
    bm.bih.xpixelpermeter = 0x130B; //2835 , 72 DPI
    bm.bih.numcolorspallette = colormapSize;
